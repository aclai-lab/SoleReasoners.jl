...
rng = Xoshiro(42)
randombranch(_::LabeledTableau) = rand(rng, Int)
@test_nowarn MetricHeap(randombranch)

################################################################################
# Logger #######################################################################
################################################################################
metricheap = MetricHeap(randombranch)
b = IOBuffer()
print(b, metricheap)
@test String(take!(b)) == "Metric: randombranch\nBinaryHeap: " *
...
@test_nowarn push!(metricheap, t0)
@test_nowarn push!(metricheap, t1)
@test_nowarn push!(metricheap, t2)
@test_nowarn push!(metricheap, t3)
@test_nowarn push!(metricheap, t4)
print(b, metricheap)
@test String(take!(b)) == "Metric: randombranch\nBinaryHeap: " *
